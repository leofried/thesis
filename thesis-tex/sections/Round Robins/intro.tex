...
% Its natural to think of Definition \ref{def:rrdef} as saying that after all of the games are played, a partial ordering is imposed on the teams such that $t_1 > t_2$ if and only if $t_1$ won more games than $t_2.$ However, we in the tournament design business are often interested in assigning total orders to teams at the conclusion of a tournament: if two (or more) teams finish the tournament with the most game wins, who wins the tournament? This is where \textit{tiebreakers} come in.
...
% By using a sequence of tiebreakers, then, we can take the partial ordering on the teams guaranteed to us by the definition of a round robin, and sequentially apply tiebreakers to various sets of incomparable teams until we get a total ordering.
}