...
\lemm{}{
    If $\A = \bracksig{a_0; ...; a_r}$ is a proper reseeded bracket on $n$ teams, $\T + \{x, y\}$ is an SST list of $n + 1$ teams, $x$ and $y$ are one seed apart in $\T + \{x, y\}$ with $x$ being the higher seed, and $z \in \T$, then $$\G{z}{y} \cdot \W{\A}{z}{\T + \{x\}} \geq \G{z}{x} \cdot \W{\A}{z}{\T + \{y\}}.$$
}{ 
...
%plausibly this is obvious and can be ignored
    % We proceed by induction on $r$. If $r = 0$, then $\A = \bracksig{1}$ and so the theorem is immediately true. For any other $r$, if $x$ would receive a first-round bye when $\A$ is run on $\T + \{x\}$ (and thus as would $y$ when $\A$ is run on $\T + \{y\}),$ then the statement follows immediately by induction. \\
