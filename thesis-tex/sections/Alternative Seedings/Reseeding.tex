...
Edwards's Theorem naturally raises the question: is there some tournament format that mantains the basic idea of the bracket, but expands the space of signatures that are ordered. \textit{Reseeded} brackets are our first attempt at an answer.
...
Both National Football League conferences use a reseeded bracket with signature $\bracksigr{6; 1; 0; 0}.$ (The superscript $R$ indicates this is reseeded bracket.) If the first round of the bracket goes chalk, then it looks just like a normal bracket:
...
\theo{}{
    ...
    This be shown by computing the probability of each team winning the format and then applying the SST conditions to establish the inequalities, as we did in Theorem \ref{th:four_ordered}. In the interest of brevity, however, we instead give an intuitive argument.\\
...
\theo{}{
    ...
    Let $\A$ be a bracket signature that cannot be generated by the procedure. Then, either there is a round in which three or more games are to be played, or there is a round in which two games and the next two rounds each have two games played as well.\\
