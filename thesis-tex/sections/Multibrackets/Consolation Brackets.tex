\sub {
    ...
    \fig{0.7}{afl_second}{The 2015 AFC Asian Cup Alternative}
    ...
    \phig{socon1}{2023 Southern Conference Wrestling Championships Alternative}

    (As discussed earlier, we opt to both the $E$-round winners in fourth, to ensure that no more than $m$ teams can claim a top-$m$ finish for any $m$.)

    The four formats presented thus far are examples of \i{multibrackets}.
    ...
    \end{definition} %maybe let's get rid of the specification of places -- i don't want to have to do it with linear s its gross

    Since which game each team plays in next (and which place each team ends up in) can be derived only from which game that team played in most recently and whether they won or lost that game, this definition is equivalent to saying that the format upholds the network condition from Section \ref{sec:Definitions}.
    ...
    We will see in the coming sections that many formats, including efficient formats, swiss systems, Page-McIntyre systems, and double-elimination tournaments are all examples of multibrackets.
}
...
% \end{itemize} %where to include actual definition
