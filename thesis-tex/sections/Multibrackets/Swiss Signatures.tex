\sub {
    ...
    Consider the 1988 Men's College Basketball Maui Invitational \cite{wiki_maui}, which used a respectful linear multibracket of signature $\bracksig{8;0;0;0} \to \bracksig{1} \to \bracksig{2;0} \to \bracksig{1} \to \bracksig{4;0;0} \to \bracksig{1} \to \bracksig{2;0} \to \bracksig{1}.$
    ...
    The format used in the Maui Invitational is of course respectful, but its flowchart has several additional properties that even the respectful flowchart in Figure \ref{fig:mlq_flow1} from last section does not have.
    ...
    There is one other nice property that the 1988 Men's College Basketball Maui Invitational Flowchart has: all of its semibrackets are either trivial or competitive. A nontrivial noncompetitive semibracket of signature $\bracksig{a_1; ...; a_r}_m$ can always be split into the the pair of semibrackets $\bracksig{a_1; ...; a_{r-1}; 0}_{m-a_r} \to \bracksig{a_r}_{a_r}$ without affecting the games played in the tournament or who ends up ranked, and so to avoid double counting formats we require this property as well.
    ...
    \writedef{Swiss Format}{
        A \i{Swiss format} is a respectful linear multibracket with the following additional properties:
        \begin{enumerate}[(a)]
    ...
    Thus $\bracksig{8;0;0;0} \to \bracksig{1} \to \bracksig{2;0} \to \bracksig{1} \to \bracksig{4;0;0} \to \bracksig{1} \to \bracksig{2;0} \to \bracksig{1}$ is a $3$-round Swiss signature. In fact, it is an example of a particular family of Swiss signatures known as the the \i{standard Swiss signatures}, which we abbreviate by $\S_r$ for some $r$.
    ...
    (McGarry and Schutz \cite{four_five_swap} considered outright swapping the positions of the fourth- and fifth-place teams at the conclusion of $\S_3$, but this format is not respectful and provides some incentive for losing in the first round in order to get an easier path to a top-half finish. Simply using $\T_3$ when identifying the top-four teams is preferable.)
