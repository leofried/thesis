\sub{
    ...
    \begin{definition}{Weakly Proper Multibracket}{}
        A multibracket $\A$ is \textit{weakly proper} if, for each bracket $\A_i$ in $\A$, teams that lost more recently are given at least many byes in $\A$ as teams that lost less recently (where rounds later in the alphabet are more recent, and if a team doesn't make a multibracket at all, this is considered negative byes.)
    \end{definition}

    Since the losers of $\bracklabel{B1}$ and $\bracklabel{B2}$ lost more recently than any of the losers of $\bracklabel{A1}$ through $\bracklabel{A4}$, if the Asian Cup wants to be weakly proper, its third-place game must be played between the losers of $\bracklabel{B1}$ and $\bracklabel{B2}$.
