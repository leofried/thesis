\sub{
    ...
    \begin{definition}{Multibracket}{multi}
        A \textit{multibracket} $\A$ is defined by a sequence of semibrackets $$\A_1 \to ... \to \A_k,$$ and played out according to the following algorithm:\\
        
        \begin{algorithmic}
            \State {$\T_1 \gets$ a list of lists such that $\T_1[i] = [\T[i]]$} 
            % \Comment {$\T_1$ is a list of lists.}

            \For{$i \gets 1$ to $k$}
                \State {$r \gets$ the number of rounds in $\A_i$}
                \State {$n \gets$ the number of teams in $\A_i$}
                \State {$\S_0 \gets$ the initial segment of $\T_i$ s.t. $|\tbreak{Flatten}(\S_0)| = n$ \Comment{$(\star)$}}
                \State {$\S_1 \gets$ the rest of $\T_i$}
                \State {$\S_0 \gets$ \tbreak{Flatten}($\S_0$)}
                    % \Comment{Transform it from a list of lists into just a list}
                \State {Play $\A_i$ on $\S_0$}
                \State {$\S_2 \gets$ a list of lists s.t.$\ \S_2[j]$ are the teams that lost in round $r - j$}
                \State {$\T_{i+1} \gets \S_2 \ @ \  \S_1$} 
                    % \Comment {$@$ is concatenation.}
            \EndFor
            \State{}
        \end{algorithmic}
        ...
        %     \item If the $i$-seed starts in $\A_j$, then the loser of each game in $\A_\ell$ for $\ell < j$ is sent to $\A_m$ for $\ell < m \leq j.$
        %     \item If the $i$-seed starts in $\A_j$, and $\ell < i$, then the $\ell$-seed starts in $\A_m$ for $m < j.$
...
%     The first requirement ensures that teams are not playing in multiple semibrackets simultaneously, and the last requirement allows us to avoid having to consider somewhat pathological semitrivial semibrackets. The other five requirements ensure that it is always better to win games than to lose them: losing games should never put you in an earlier bracket than winning them, even accounting for the fact that future games in earlier brackets will likely be against better teams then future games in later brackets.

%     Note that the definition the multibracket allows multiple teams to finish in the same place. For example, the multibracket of signature $\bracksig{8;0;0;0} \to \bracksig{3}_3$ crowns one team champion and grants fourth-place to three teams. However,

%     \theo{}{
%         No more than $m$ teams in a multibracket finish in the top-$m$.
%     }{
%         This follows from how placed finishes are defined in Definition \ref{def:multi}.
%     }{}

%     %why can we remove semitrivial brackets?

%     Colloquially, we use the terms \textit{higher} and \textit{lower} to refer to certain semibrackets in a multibracket.
...
%     With the general format of the multibracket established, the next few sections will examine particular classes of the design.
% }