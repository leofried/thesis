\sub{
    ...
    It might be tempting to think of the 2015 AFL Asian Cup as a semibracket. After all, semibrackets look like multiple brackets being played concurrently when drawn out. Unfortunately, the 2015 AFL Asian Cup doesn't quite fit into the definition of a semibracket: for one thing, the losers of games $\bracklabel{B1}$ and $\bracklabel{B2}$ continue playing even after they lose a game.
    
    Instead, the 2015 AFL Asian Cup is a \textit{multibracket}: a sequence of brackets (or semibrackets) in which teams that lose in earlier brackets fall into later brackets instead of being eliminated outright, and teams finish in a place dependent on which bracket they win. Formally,
