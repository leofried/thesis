\sub {
    ...
    This construction, where rather than a contingent game being played only if it is not a rematch, it is played only if the lower team wins, is so common that it has a name.
    ...
    \theo{}{ %make sure all the notation (primary, etc) is cover --> add a picture to different winners nad losers brackets
        ...
        Let $\A$ be the balanced bracket on $2^n$ team, and let $\B$ be a round-respectful double-elimination tournament whose winners bracket is $\A,$ with or without a recharge game. Then $$\W{\A}{t}{\T} \leq \W{\B}{t}{\T}$$ with equality only when $n=1$ and there is no recharge round.
        %technically round respectful doesn't mean anything!
    }{
        To win $\A$, $t$ simply has to win $n$ games. Thus $$\W{\A}{t}{\T} = p^n.$$
    
        We don't know what the losers' bracket of $\B$ looks like, only that it is round-respectful. Let $r$ be the number of rounds in the losers' bracket, and let $c_i$ be the number of games that $i$-round winners' bracket losers need to win the losers' bracket.\\
        ...
        $$\W{\B}{t}{\T} \leq p^n\cdot p + p^n \cdot q \cdot p + qp^{n-1}\cdot p^2 = p^n(p + qp + p^2) > p^n,= \W{\A}{t}{\T}$$ with the inequality coming because $p > \frac{1}{2}.$ If there is no recharge round, then 
        $$\W{\B}{t}{\T} = p^n\cdot p + qp^{n-1}\cdot p = p^n(p + qp + p^2) > p^n,= \W{\A}{t}{\T}$$

        % You get it, with $\C$ and $\B$ and stuff lol
    }{}
