\sub {
    ...
    \theo{}{
        ...
        Assume now that no such $j$ exists, so there exists some some $i$ such that $\A_i$ is nontrivial and $\A_{i+1}$ is noncompetitive. $\A_{i+1}$ is noncompetitive, so at least one team must win $\A_{i+1}$ without playing a game. Because $\A$ is proper, this team must be a championship game loser of $\A_i$, if such a game exists. $\A_i$ is nontrivial so such a game does indeed exist: let $\bracklabel{G}$ be that game. Then the winner of $\bracklabel{G}$ wins $\A_i$, and the loser of $\bracklabel{G}$ wins $\A_{i+1}$, so $\A$ is not weakly efficient.
    }{w_efficient}
