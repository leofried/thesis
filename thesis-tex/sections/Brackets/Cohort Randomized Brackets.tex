\sub {
    ...
    Before each Wimbledon Mixed Double's Championship, the seeding is constructed in the following way: the 1- and 2-seeds are placed normally. Then, half the time the 3- and 4-seeds are placed normally, and half the time they are swapped. Then the 5- through 8-seeds are randomly placed into the four starting lines that are normally assigned to those four seeds. Finally, the remaining 24 teams are placed randomly into the remaining spots.
    ...
    \theo{}{
        ...
        We proceed by induction on $r.$ If $r = 0$, then $\A = \bracksig{1}$, and so the theorem holds. For any other $r$, in the first round, $a_0$ teams are playing $b_1 = a_0 / 2$ games. However, the $b_1$ lowest seeds in the tournament are shuffled between themselves, so none of them can play each other. Because the tournament goes chalk, they will all lose, so after the first round, the $n - b_1$ remaining teams are the top $n - b_1$ top seeds. Further, we are left with the chalk randomized bracket of signature $\SS(\A)$, so the theorem holds by induction.
    }{}
    ...
    Chalk randomized balanced brackets are of nice enough form that we can show their orderedness:
