...
\writedef{Bracket Signature}{
    The \i{signature} of an $r$-round bracket $\A$ is the list $\bracksig{a_0; ...; a_r}$ where
    %for  $0 \leq i \leq r$, 
    $a_i$ is the number of teams that get $i$ byes.
...
\theo{theorem}{
    ...
    A bracket is balanced if no teams have byes, which is true exactly when its signature is of the form $\A = \bracksig{n; 0; ...; 0},$ where $n$ is the number of teams in the bracket. By Theorem \ref{th:signature_sum}, $\A$ is a bracket signature only when $n$ is a power of two, and so there exists an $n$-team balanced bracket if and only if $n$ is a power of two.
}{balanced}{\unattributed}
