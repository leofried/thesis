\sub {
    ...
    \theo{lemma}{
        ...
        Let $X$ be the power set of $\S \setminus \{s\} = T \setminus \{t\}$, and for each set of teams $Y \in X$, let $P_Y$ be the probability that $s$ or $t$ will have to beat exactly the set of teams $Y$ in order to win the tournament (noting that this probability is the same for $s$ and $t$). Then,
        \begin{align*}
            \W{\A}{t}{\T} &= \sum_{Y \in X} \left(P_Y \cdot \prod_{u \in Y} \G{t}{u}\right)\\
            &\geq \sum_{Y \in X} \left(P_Y \cdot \prod_{u \in Y} \G{s}{u}\right)\\
            &= \W\A s\S \end{align*} }{rand_lemma}{Chen and Hwang, 1988}
