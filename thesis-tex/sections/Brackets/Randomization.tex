\sub {
    ...
    \lemm{}{
        Let $\A$ be a totally randomized knockout tournament with signature $\bracksig{a_0; ...; a_r}$, let $\S$ be a list of teams, and let $\T$ be the list of teams produced by replacing a given team $s \in \S$ with a team $t$ such that for all other teams $u,$ $$\G{t}{u} \geq \G{s}{u}.$$ Then, $$\W{\A}{t}{\T} \geq \W{\A}{s}{\S}.$$
    }{
    ...
    But the evidence for Conjecture \ref{conj:randord} isn't that strong: Lemma \ref{th:rand_lemma} is true for all traditional brackets as well, even though they are not always ordered. Indeed, despite the lemma, Conjecture \ref{conj:randord} turns out to be false. This was first shown by Israel \cite{seventeen_team} with the following counterexample.
