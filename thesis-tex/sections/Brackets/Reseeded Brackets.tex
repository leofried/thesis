\sub{
    ...
    Edwards's Theorem tells us that the number of rounds required to construct on ordered bracket grows linearly with the number of teams involved. This can be somewhat frustrating, as part of the power of brackets is the ability to crown a champion in a number of rounds logorithmic in the number of teams participating. But what if we briefly looked at a wider range of formats, those that are similar to brackets but not necessarily networked. Can we recover some ordered bracket-like formats that way?

    \begin{definition}{Knockout Tournament}{}
        ...
        \begin{enumerate}[(a)]
            ...
            \item If a team wins a round, they play a game in the next round.
            \item Exactly one team finishes undefeated, and that team is crowned champion.
        \end{enumerate} 
    \end{definition}

    Clearly brackets are just networked knockout tournaments, but there are many knockout tournaments that aren't networked. The definition of a knockout tournament is designed to allow for the definitions of byes, signatures, and properness to still apply.

    \begin{definition}{Bye}{}
        A team has a \i{bye} in round $r$ if it plays no games in round $r$ or before.
    \end{definition}

    \begin{definition}{Knockout Tournament Signature}{}
    ...
    \begin{definition}{Proper Knockout Tournament}{}
