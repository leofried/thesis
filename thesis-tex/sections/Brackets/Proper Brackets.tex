...
\begin{definition}{Higher and Lower Seeds}{}
    Somewhat confusingly, convention is that smaller numbers are the \textit{higher seeds}, and greater numbers are the \textit{lower seeds}.
\end{definition}
...
\theo{The Fundamental Theorem of Brackets}{There is exactly one proper bracket with each bracket signature.}{
    ...
    For any other $r$, the first-round matchups of a proper bracket with signature $\A$ are defined by Theorem \ref{th:proper_condition_two}. Then if those matchups go chalk, we are left with a proper bracket of signature $\SS(\A)$, which induction tells us exists admits exactly one proper bracket.\\
