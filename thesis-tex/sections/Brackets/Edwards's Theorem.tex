...
\fig{.85}{combine}{The Setup of Theorem \ref{th:order_bracket_sum}}
...
\lemm{}{
    ...
    Now, assume for contradiction that two games are played in round $s$ but the winners of those games do not play each other in the following round. So let $k<\ell$ such that, if the bracket goes chalk, $t_k$ plays $t_\ell$ in round $s$, but the winner of that game doesn't play the winner of the $t_m$ vs $t_{m+1}$ game in round $s+1.$ In fact, let $k$ be the lowest such seed. Finally, let $i$ and $j$ be such that if the bracket goes chalk, in round $s + 1$, $t_m$ will play $t_i$ and $t_k$ will play $t_j.$\\

    The situation so far (assuming all omitted games go chalk):
    \begin{center}
        \graph{0.7}{two_must_play}
    \end{center}
        
    We can use $\A$'s properness to determine that $i < j < k < m < m + 1 < \ell.$\\

    Consider now the following SST matchup table (empty cells below the diagonal are zeros, and empty cells above the diagonal are ones.):

    \begin{center}
        \graph{0.7}{edward_matchup_table}
    \end{center}
