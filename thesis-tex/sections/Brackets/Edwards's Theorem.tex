\sub {
    ...
    \ntheo{lemma}{The Containment Lemma}{
        ...
        Let $\A$ be a bracket signature with $r$ rounds and $n$ teams, and let $\B$ have $s$ round and $m$ teams, such that $\A$ contains $\B$ and $\B$ is not ordered. Let $k$ be the number of teams in $\A$ that get at least $s + i$ byes (where $i$ is from the definition of \i{contains}).\\
