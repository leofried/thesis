...
\begin{definition}{Gameplay Function}{}
    A \textit{gameplay function} $g$ on a list of teams $\T = \{t_1, ..., t_n\}$ is a nondeterministic function $g : \T \times \T \to \T$ with the following properties:
        \begin{itemize}
...
\begin{definition}{Playing, Winning, and Losing}{}
    When $g$ is queried on input $(t_i, t_j)$ we say that $t_i$ and $t_j$ \textit{played a game}. We say that the team that got outputted by $g$ \textit{won}, and the team that did not \textit{lost}.
\end{definition}

The information in a gameplay function can be encoded into a \textit{matchup table.}

\begin{definition}{Matchup Table}{}
    The \textit{matchup table} implied by a gameplay function $g$ on a list of teams $\T$ of length $n$ is a $n$-by-$n$ matrix $\M$ such that $\M_{ij} = \G{t_i}{t_j}.$
\end{definition}

For example, let $\T = \{$Favorites, Rock, Paper, Scissors, Conceders$\}$, and $g$ be such that the Conceders concede every game they play, the Favorites are $70\%$ favorites against Rock, Paper, and Scissors, and Rock, Paper, and Scissors matchup with each other as their name implies. Then the matchup table would look like so:
