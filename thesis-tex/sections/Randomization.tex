\sub {
    ...
    \lemm{}{
        ...
        Let $X$ be the set of subsets of $\S \setminus \{s\} = T \setminus \{t\}$, and for each set of teams $Y \subset X$, let $P_Y$ be the probability that $s$ or $t$ will have to beat exactly the set of teams $Y$ in order to win the format (noting that this probability is the same for $s$ and $t$). Then,
        \begin{align*}
    ...
    Thus the 1- and 2-seeds are locked into their places, the 3- and 4-seeds exchange places half the time, seeds 5-8 are randomly shuffled, and as are 9-16, 17-32, etc. An example of what a cohort randomized bracket might look after randomization is displayed in Figure \ref{fig:16rcb}
...
%     }{
%         Let $\A$ be the eight-team cohort randomized brakcte, and let $\T$ have the following matchup table for $0 < p < 0.5.$
...
%         We will show that $\W_{\A}{t_2}{\T} < \W_{\A}{t_3}{\T}.$ Note first that the half the time when $t_3$ falls on the same side of the bracket as $t_2$, they have the same win probability for their first round, semifinal, and finals games, and thus the same win probability overall. Thus, we let $\B$ be the format identical to $\A$ except $t_3$ is guaranteed to be placed on the starting line traditional given to the 4-seed and look to show that
%         $$\W_{\B}{t_2}{\T} < \W_{\B}{t_3}{\T}.$$



%         % Now, note that for all $i$, $p_{2i} = p_{3i} = p_{4i}$ and $p_{6i} = p_{7i} = p_{8i}.$ Thus, the only rand

% %         Define
% %         \begin{align*}
% %             q &= 1 - p\\ %X_D
% %             r &= pq + q\frac{1}{2}\\ %Y_1D, Y_BD
% %             s &= \frac{1}{4} + \frac{1}{2}q\\ %Y_5D
% %             t &= q\frac{1}{2} + p(qr + ps)\\%Y_BD15
% %             u &= q\frac{1}{2} + p(\frac{1}{2}r + \frac{1}{2}s)\\%Y_1DB5, Y_BDB5,
% %             v &= q^{2}\frac{1}{2}+2pqr+p^{2}q\\
% %             w &= \frac{1}{2} - p = 1 - \frac{1}{2}%Y_1DBD,  Y_BDBD
% %         \end{align*}
% %             % u &= q\frac{1}{2}+p\frac{1}{2}\left(r\right)
% %             % v &= q\frac{1}{2} + p\left(qr + p\left(\frac{1}{4} + \frac{1}{2}\right)\right)\\
% %   %          
% %             % q^2\frac{1}{2} + qpr + pq\f
% %  %           
% %             % \frac{1}{2}q^2 + pqr + \frac{1}{2}pq + p^2(1/4 + 1/2q) = 
% % %
% % %
% % %
% %             % s &= \frac{1}{2}q^{2}+2pqr+p^{2}q\\
% %             % t &= \frac{q}{2}+\frac{p}{2}\left(r+\frac{1}{4}+\frac{q}{2}\right)\\
% %             % u &= \frac{1}{2}q+pqr+p^{2}\left(\frac{1}{4}+\frac{q}{2}\right)
  

% %         Let's first calculate $\W{\A}{t_2}{\T}.$ Note that $t_3$ and $t_4$ have the same matchup probabilities, and $t_6, t_7, t_8$ do as well, so the only randomization property that is relavent is where $t_5$ is placed.Thus,
% %         \begin{align*}
% %             \W{\A}{t_2}{\T} &= \frac{1}{4}(qrt + qru + q\frac{1}{2}v + \frac{1}{2}rv)
% %         \end{align*}

% %         Now, when calculating $\W{\A}{t_3}{\T}$ we note that half the time, when $t_3$ is put on the same side of the bracket as $t_2$, they will win the format at the same rate as $t_2.$ The other half the time, we must calculate it by hand. Thus,
% %         \begin{align*}
% %             \W{\A}{t_3}{\T} &= \frac{1}{8}(qrt + qru + q\frac{1}{2}v + \frac{1}{2}rv + q\frac{1}{2}v + \frac{1}{2}rv + 2qru).\\
% %         \end{align*}
% %         Comparing, we find
% %         \begin{align*}
% %             \W{\A}{t_2}{\T} - \W{\A}{t_3}{\T}  &= \frac{1}{8}(qrt - qru)\\
% %             &= \frac{qr}{8}(t - u)\\
% %             &= \frac{qr}{8}(q\frac{1}{2} + p(qr + ps) - q\frac{1}{2} + p(\frac{1}{2}r + \frac{1}{2}s))\\
% %             &= \frac{pqr}{8}(qr+ps-\frac{1}{2}r-\frac{1}{2}s)\\
% %             &= \frac{pqrw}{8}(r-s)\\
% %             &= \frac{pqrw}{8}(pq + q\frac{1}{2}-\frac{1}{4} - \frac{1}{2}q)\\
% %             &= \frac{pqrw}{8}(pq + q\frac{1}{2}-\frac{1}{4} - \frac{1}{2}q)\\
% %             &= \frac{pqrw}{8}(pq-\frac{1}{4} )\\
% %             &<0
% %         \end{align*}

% %         Thus $\W{\A}{t_2}{\T} < \W{\A}{t_3}{\T}$, so $\A$ is not ordered.

% %         % Let $q = 1 - p.$ Let $r = pq + \frac{1}{2}q$ Noting that $r$ is the probability that $t_2$ beats the winner of a game between $t_i$ and $t_j$ where $i, j \in \{1, 3, 4\} \times \{6, 7, 8\}.$ Let $s = 
...
%     }{}
%     %     The cohort randomizations of balanced brackets have particularly nice forms: the cohort randomization of an $r$-round balanced bracket is parameterized by $$(\bracksig{2^r; 0; ...; 0}, \bracktier{2^{r-1} + 2^{r-2} + ... + 2^{0} + 1}).$$
    
%     %     This is very similar to the Wimbledon format, which employs a cohort randomized balanced bracket but merges the first two cohorts. Wimbledon even uses this scheme in their single's tournaments, which are on 128 teams and are parameterized by $$(\bracksig{128; 0; ...; 0}, \bracktier{96 + 16 + 8 + 4 + 2 + 1}).$$
...
%     % While totally randomized (balanced) brackets solves the orderedness problem that Edwards's Theorem presents, it does so at a significant cost: excitment. In Section \ref{sec:Proper Brackets}, we saw Lemma \ref{th:proper_condition_one_point_five}, which guarenteed that if a proper bracket goes to chalk, then after each round, the $m$ remaining teams would be the top $m$ seeds. This ensures that the later rounds of a proper bracket would likely consist of increasingly exciting game, culminating in the championship game which would be between the top two seeds. totally randomized brackets provide no such guarentee: the top two seeds could meet each other in the first round if we get partiularly unlikely.
...
%     % \i{Cohort Randomized Brackets}, a variant of which is used in tournaments on the the Assosiation for Tennis Profesionals Tour, and initially named by Schwenk.

%     % \begin{definition}{Cohort Randomized Bracket}{}
%     %     The \i{cohort randomized bracket} on $2^r$ teams is the traditional balanced bracket on $2^r$ teams, except, for each $i$, seeds $2^i + 1$ through $2^{i+1}$ are shuffled randomly before play.
%     % \end{definition}
...
%     % Cohort Randomization looks to use randomization to maintain its orderedness like totally randomized brakcet
