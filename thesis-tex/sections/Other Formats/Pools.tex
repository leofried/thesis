\sub {
    ...
    % Consider the 2019 FIFA Women's World Cup Baseball Classic. Twenty teams  qualified for the Classic, and they were divided into four pools of five teams each. Each pool played out a self-contained round robin. Then, the bottom three teams in each pool were eliminated, and the remaining eight teams (two from each pool) played out the multibracket $\bracksig{8;0;0;0;0}.$ The entire format as it played out is displayed in Figure \ref{fig:world_cup}.


}
    %We should be using the world cup because it has a third-place game and is thus a multibracket



%     Consider the 2023 World Baseball Classic. Twenty teams qualified for the Classic, and they were divided into four pools of five teams each. Each pool played out a self-contained round robin. Then, the bottom three teams in each pool were eliminated, and the remaining eight teams (two from each pool) played out the multibracket $\bracksig{8;0;0;0;0}.$ The entire format as it played out is displayed in Figure \ref{fig:world_cup}.

%     \fig{0.75}{world_cup}{The 2023 World Baseball Classic}

%     Note that the seeds in the bracket $\bracksig{8;0;0;0;0}$ are not just the numbers $1$ through $8$, but instead letter-number combination. $\bracklabel{B2}$ indicates that the second-place team from pool $\bracklabel{B}$ should be placed in that location. To avoid confusion, we start at $\bracklabel{E}$ when labeling the rounds. In alignment with the discussion in Section \ref{sec:Additional Considerations}, the 2023 World Baseball Classic attempts to avoid rematches, ensuring that the two teams that qualify to bracket from each pool can only meet again in the finals.

%     The format in Figure \ref{fig:world_cup} combines the two genres of format we've discussed thus far: the first half is a scheduled format (in particular, four separate round robins), and the second half is a multibracket. How teams perform in the first phase of the format affects how they are seeded (and whether they qualify at all) in the second. We call such formats \i{pool to bracket formats.}

%     \begin{definition}{Pool to Bracket}{}
%         A \i{pool to bracket} is a tournament format in which teams are divided amongst a number of pools, each pool plays a round robin, and then the results of the round robins are used to place (some) of the teams into a multibracket which is then played out to get the final placements. We require that if teams $t_1$ and $t_2$ share a pool, and $t_1$ places higher in the pool, then $t_1$ starts in at least as high of a semibracket than $t_2.$
%     \end{definition}
...
%     The signature of the 2023 World Baseball Classic is $\poolsig{8x4} \to \bracksig{16;0;0;0;0}.$
...
%         %Plausibly we should also have thoughts about how to divide the seeds ()
% }