...
\theo{The Fundamental Theorem of Brackets}{There is exactly one proper bracket with each bracket signature.}{
    ...
    For any other $r$, the first-round matchups of a proper bracket with signature $A$ are defined by Lemma \ref{th:proper_condition_two}. Then if those matchups go to chalk, we are left with a proper bracket with signature $B = [a_1 + a_0/2; a_2; ...; a_r]$, which induction tells us exists admits exactly one proper bracket.\\
